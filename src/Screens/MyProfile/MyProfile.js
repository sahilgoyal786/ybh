...
import {Text,ScrollView,View,Image,ActivityIndicator,TouchableOpacity} from 'react-native';
    ...
    LoadProfile = (userToken) => {
        ...
        try{
            network.getResponse(
                ...
                userToken,
                (response) => {
                    this.setState({profile: response,token: userToken,isLoading: false});
                },
                (error) => {
        ...
        return (
            <View style={{flex: 1,backgroundColor: '#fff'}}>
                ...
                <ScrollView alwaysBounceHorizontal={false} alwaysBounceVertical={false} bounces={false} style={{padding:30,paddingTop:20}} contentContainerStyle={{paddingBottom:40}}>
                    <UserProfileWrap>
                        <UserImage source={this.state.profile.photo ? {uri: this.state.profile.photo} : placeholderProfilePhoto} resizeMode="cover"/>
                        ...
                        <TouchableOpacity onPress={() => navigation.navigate('EditMyProfile')}>
                            <View style={{padding: 5,backgroundColor: '#7b43a5',borderRadius: 5,paddingLeft: 25,paddingRight: 25,marginTop: 15}}>
