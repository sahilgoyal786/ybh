...
class Plans extends React.Component {
    ...
    componentDidMount() {
        ...
        if(user.length){
            this.updateAccessToken(user[0].token,user[0].user.subscription_is_active);
        }
    }
    ...
    redirectToPayment = () => {
        ...
        if(this.state.subscription){
            ...
            Linking.openURL(this.state.paymenturl).catch((err) => console.error('Error', err));
        }
    }
    checkpaymentStatus = () => {
        const user = this.context;
        ...
        try{
            network.getResponse(
                ...
                this.state.token,
                (response) => {
                    this.setState({subscription: response.subscription,isLoading: false});
                    if(response.subscription){
                        user[0].user.subscription_is_active = response.subscription;
                        Toast.show({text: `You have puchased Subscription plan.`,type: 'success'});
                    }else{
                        Toast.show({text: `You don't have any Subscription plan.`,type: 'danger'});
                    }
                },
                (error) => {
    ...
    render() {
        ...
        return (
            <View style={{flex: 1,height: '100%'}}>
                ...
                <Image source={PlanBackground} style={{position:'absolute',left:0,right:0,top:0,bottom:0}} resizeMode="contain"></Image>
                <BoxView>
                    ...
                    <ViewBox>
                        ...
                        {this.state.subscription && (
                            <Button onPress={() => navigation.navigate('PhotoVerification')} style={{width:'100%',marginTop:25}} name={'Continue'} linear/>
                        )}
                        {!this.state.subscription && (
                            <Button onPress={() => this.redirectToPayment()} style={{width:'100%',marginTop:25}} name={'Buy Now!'} linear/>
                        )}
                        <Text style={{ marginTop: 5, padding: 5}} onPress={() => navigation.navigate('Welcomeuser')}>Go Back</Text>
                        <ReloadBtn onPress={() => this.checkpaymentStatus()}>Check Payment Status</ReloadBtn>
                    </ViewBox>
