  ...
  render() {
    return (
      <View style={{flex:1,backgroundColor:'white'}}>
        ...
        <Header title="Photo Verification" backButton="true" showRightDrawer={false}/>
        <ScrollView alwaysBounceHorizontal={false} alwaysBounceVertical={false} bounces={false} style={{padding:40,paddingTop:20}} contentContainerStyle={{paddingBottom: 40}}>
          <View style={{marginBottom: 20,alignItems: 'center'}}>
            <Heading>Keep your face still and look into the circle.</Heading>
            <Text style={{fontSize:16,lineHeight:'24px'}}>Without verifying your photo you cannot use this app. We have to make sure is a safe environment.</Text>
            <ProfileImage source={this.state.profile_photo ? {uri: 'https://api.ybhive.app/storage/'+this.state.profile_photo} : placeholderProfilePhoto} />
            <Text style={{fontSize:17,lineHeight:'24px',textAlign:'center'}}>Your photo will not be posted or stored once verified.</Text>
            <Button onPress={() => this.openCamera()} style={{width: '100%',marginTop: 40}} name={'Take Picture to Verify'} linear/>
          </View>
        </ScrollView>
      </View>
    );
