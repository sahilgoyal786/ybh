...
import {Text,ScrollView,View,Image,TouchableWithoutFeedback,ActivityIndicator} from 'react-native';
  ...
  LoadConnections = (token) => {
    try {
      network.getResponse(
        ...
        token,
        (response) => {
          this.setState({profiles: response.data,token: token,isLoading: false});
        },
        (error) => {
  ...
  createChats = (userId) => {
    ...
    try{
      network.getResponse(
        ...
        (response) => {
          this.setState({isLoading: false,showPopover: false});
          navigation.navigate('ChatMessage',{chat_id: response.id,name: response.user.username,receiver: response.user.user_id,photo: response.user.photo});
        },
        (error) => {
          this.setState({isLoading: false,showPopover: false});
          console.log('error',error);
        ...
        if(this.state.showPopover === item.id){
          popup.push(
            <Popover key={'popup'+index}>
              <TouchableWithoutFeedback onPress={() => this.disconnectRequest(item.id,index)}>
                <PopoverBtn style={{borderBottomWidth: 1,borderColor:'#ddd'}}>
                  <IconImage source={UserProfileIcons['delete']} resizeMode="contain"></IconImage>
                  <Text>Remove</Text>
                </PopoverBtn>
              </TouchableWithoutFeedback>
              <TouchableWithoutFeedback onPress={() => this.createChats(item.user_id)}>
                <PopoverBtn>
                  <IconImage source={UserProfileIcons['colormessage']} resizeMode="contain"></IconImage>
                  <Text>Message</Text>
                </PopoverBtn>
              </TouchableWithoutFeedback>
            </Popover>
        ...
        profileLists.push(
          <UserList key={index}>
            <UserListInner>
              ...
              {popup}
              <UserImage source={{uri: item.photo}} resizeMode="cover"></UserImage>
              <UserData>
                ...
                <UserMessage>{item.state}, {item.country}, {item.age}</UserMessage>
              </UserData>
            </UserListInner>
          </UserList>
        );
